...
import numpy as np
from noise_rate_estimation import noise_rate_estimation
...
def get_noise(data_dir):
    with open(os.path.join(data_dir, "noise.t"), 'r') as f:
        lines = f.read().split('\n')
        noise_ds, noise_sd = float(lines[0]), float(lines[1])
...
class CustomLoss(nn.Module):
    ...
    def __init__(self, pi_p_eta, pi_s, pi_s_eta, noiseds, noisesd, loss_name):
        super(CustomLoss, self).__init__()
        self.pi_p_eta = pi_p_eta
        self.pi_s = pi_s
        self.pi_s_eta = pi_s_eta
        self.noiseds = noiseds
        self.noisesd = noisesd
        self.lname = loss_name
        self.compute_C()

    def compute_C(self):
        nm = self.pi_s_eta * ((1 - self.noiseds) * (1 - self.pi_s) + self.noisesd * self.pi_s)
        dn = (1 - self.pi_s_eta) * (self.noiseds * (1 - self.pi_s) + (1 - self.noisesd) * self.pi_s)
        self.C = nm / dn
    ...
    def forward(self, output, target):
        ...
        L0 = torch.where(target == 1, 0.0, self.C) * L0
...
def training(ds, dir_path, num_epochs, runs, batch_size, lr, k, pi_p_eta, pi_s, pi_s_eta, num_features, noiseds, noisesd, lname):    

    f_path = os.path.join(dir_path, 'output.log')
    r_path = os.path.join(dir_path, 'report.log')

    print("Noise (D|S, S|D): {}, {}".format(noiseds, noisesd), flush=True)
    print("Noisy Prior (PI_P_ETA, PI_S_ETA): {}, {}".format(pi_p_eta, pi_s_eta), flush=True)
    print("Prior (PI_S): {}".format(pi_s), flush=True)
    ...
    for run in range(runs):
        ...
        for fold, (tr_id, val_id) in enumerate(splits.split(np.arange(len(ds)))):
            ...
            criterion = CustomLoss(pi_p_eta, pi_s, pi_s_eta, noiseds, noisesd, lname)
...
if __name__ == '__main__':
    ...
    if FULL == 0:
        NOISE_DS, NOISE_SD = get_noise(data_dir)
    else:
        NOISE_DS, NOISE_SD = noise_rate_estimation(ds)

    PI_S = (NOISE_SD * PI_D_ETA - (1 - NOISE_SD) * PI_S_ETA) / (NOISE_SD * NOISE_DS - (1 - NOISE_DS) * (1 - NOISE_SD))
    PI_D = 1 - PI_S
    PI_P_ETA = (1 + (2 * PI_S_ETA - 1)**0.5) / 2
    PI_N_ETA = 1 - PI_P_ETA

    dir_path = os.path.join(save_dir, str(str(NOISE_DS) + "_" + str(NOISE_SD)))
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)

    training(ds, dir_path, EPOCHS, RUNS, BATCHSIZE, LR, K, PI_P_ETA, PI_S, PI_S_ETA, NUM_FEATURES, NOISE_DS, NOISE_SD, LNAME)
